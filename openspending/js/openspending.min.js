 ...
 * limitations under the License.
 */
;var OpenSpending=OpenSpending||{};OpenSpending.Amounts=OpenSpending.Amounts||{};OpenSpending.Amounts.amountLetters={billion:"b",million:"m",thousand:"k"};OpenSpending.Amounts.shorthand=function(e){var c=1000000000;var a=1000000;var f=1000;var d=Math.abs(e);if(d>c){return OpenSpending.Amounts.format(e/c)+OpenSpending.Amounts.amountLetters("billion")}else{if(d>a){return OpenSpending.Amounts.format(e/a)+OpenSpending.Amounts.amountLetter("million")}else{if(d>f){return OpenSpending.Amounts.format(e/f)+OpenSpending.Amounts.amountLetter("thousand")}else{return OpenSpending.Amounts.format(e,2)}}}};OpenSpending.Amounts.format=function(d,c,a){a=OpenSpending.Amounts.currencySymbol(a);return accounting.formatMoney(d,a.symbol,c||0,OpenSpending.localeGroupSeparator,OpenSpending.localeDecimalSeparator,a.format)};OpenSpending.Amounts.currencySymbol=function(a){if(a){return OpenSpending.Amounts.currencySymbols[a]||{symbol:a}}else{return""}};OpenSpending.Amounts.currencySymbols={AED:{symbol:"د.إ"},AFN:{symbol:"؋"},ALL:{symbol:"L"},AMD:{symbol:"դր."},ANG:{symbol:"ƒ"},AOA:{symbol:"Kz"},ARS:{symbol:"$"},AUD:{symbol:"$"},AWG:{symbol:"ƒ"},AZN:{symbol:"m"},BAM:{symbol:"KM",format:"%v%s"},BBD:{symbol:"$"},BDT:{symbol:"৳"},BGN:{symbol:"лв"},BHD:{symbol:"ب.د"},BIF:{symbol:"Fr"},BMD:{symbol:"$"},BND:{symbol:"$"},BOB:{symbol:"Bs."},BRL:{symbol:"R$"},BSD:{symbol:"$"},BTN:{symbol:"Nu"},BWP:{symbol:"P"},BYR:{symbol:"Br"},BZD:{symbol:"$"},CAD:{symbol:"$"},CDF:{symbol:"Fr"},CHF:{symbol:"Fr"},CLP:{symbol:"$"},CNY:{symbol:"¥"},COP:{symbol:"$"},CRC:{symbol:"₡"},CUP:{symbol:"$"},CVE:{symbol:"$, Esc"},CZK:{symbol:"Kč"},DJF:{symbol:"Fr"},DKK:{symbol:"kr"},DOP:{symbol:"$"},DZD:{symbol:"د.ج"},EEK:{symbol:"KR"},EGP:{symbol:"£,ج.م"},ERN:{symbol:"Nfk"},ETB:{symbol:"Br"},EUR:{symbol:"€"},FJD:{symbol:"$"},FKP:{symbol:"£"},GBP:{symbol:"£"},GEL:{symbol:"ლ"},GHS:{symbol:"₵"},GIP:{symbol:"£"},GMD:{symbol:"D"},GNF:{symbol:"Fr"},GTQ:{symbol:"Q"},GYD:{symbol:"$"},HKD:{symbol:"$"},HNL:{symbol:"L"},HRK:{symbol:"kn"},HTG:{symbol:"G"},HUF:{symbol:"Ft"},IDR:{symbol:"Rp"},ILS:{symbol:"₪"},INR:{symbol:"₨"},IQD:{symbol:"ع.د"},IRR:{symbol:"﷼"},ISK:{symbol:"kr.",format:"%v %s"},JMD:{symbol:"$"},JOD:{symbol:"د.ا"},JPY:{symbol:"¥"},KES:{symbol:"KSh"},KGS:{symbol:"лв"},KHR:{symbol:"៛"},KMF:{symbol:"Fr"},KPW:{symbol:"₩"},KRW:{symbol:"₩"},KWD:{symbol:"د.ك"},KYD:{symbol:"$"},KZT:{symbol:"Т"},LAK:{symbol:"₭"},LBP:{symbol:"ل.ل"},LKR:{symbol:"ரூ"},LRD:{symbol:"$"},LSL:{symbol:"L"},LTL:{symbol:"Lt"},LVL:{symbol:"Ls"},LYD:{symbol:"ل.د"},MAD:{symbol:"د.م."},MDL:{symbol:"MDL"},MGA:{symbol:"Ar"},MKD:{symbol:"ден"},MMK:{symbol:"K"},MNT:{symbol:"₮"},MOP:{symbol:"P"},MRO:{symbol:"UM"},MUR:{symbol:"₨"},MVR:{symbol:"ރ."},MWK:{symbol:"MK"},MXN:{symbol:"$"},MYR:{symbol:"RM"},MZN:{symbol:"MT"},NAD:{symbol:"$"},NGN:{symbol:"₦"},NIO:{symbol:"C$"},NOK:{symbol:"kr"},NPR:{symbol:"₨"},NZD:{symbol:"$"},OMR:{symbol:"ر.ع."},PAB:{symbol:"B/."},PEN:{symbol:"S/."},PGK:{symbol:"K"},PHP:{symbol:"₱"},PKR:{symbol:"₨"},PLN:{symbol:"zł"},PYG:{symbol:"₲"},QAR:{symbol:"ر.ق"},RON:{symbol:"RON"},RSD:{symbol:"RSD"},RUB:{symbol:"р."},RWF:{symbol:"Fr"},SAR:{symbol:"ر.س"},SBD:{symbol:"$"},SCR:{symbol:"₨"},SDG:{symbol:"S$"},SEK:{symbol:"kr"},SGD:{symbol:"$"},SHP:{symbol:"£"},SLL:{symbol:"Le"},SOS:{symbol:"Sh"},SRD:{symbol:"$"},STD:{symbol:"Db"},SYP:{symbol:"£, ل.س"},SZL:{symbol:"L"},THB:{symbol:"฿"},TJS:{symbol:"ЅМ"},TMT:{symbol:"m"},TND:{symbol:"د.ت"},TOP:{symbol:"T$"},TRY:{symbol:"₤"},TTD:{symbol:"$"},TWD:{symbol:"$"},TZS:{symbol:"Sh"},UAH:{symbol:"₴"},UGX:{symbol:"Sh"},USD:{symbol:"$"},UYU:{symbol:"$"},UZS:{symbol:"лв"},VEF:{symbol:"Bs"},VND:{symbol:"₫"},VUV:{symbol:"Vt"},WST:{symbol:"T"},XAF:{symbol:"Fr"},XCD:{symbol:"$"},XOF:{symbol:"Fr"},XPF:{symbol:"Fr"},YER:{symbol:"﷼"},ZAR:{symbol:"R"},ZMK:{symbol:"ZK"},ZWL:{symbol:"$"}};
 ...
 * limitations under the License.
 */
;var OpenSpending=OpenSpending||{};OpenSpending.Taxman=function(d){var a=this;this.taxes=undefined;var c=$.extend(true,{},OpenSpending.Taxman.defaults,d);this.get=function(f){c.incomeparser(f);var e=$.getJSON(c.site+"/"+c.country+"?callback=?",c.opts).done(function(h){a.taxes=c.success(h);c.callback(a.taxes)});return e}};OpenSpending.Taxman.defaults={site:"http://taxman.openspending.org",country:undefined,opts:{},callback:function(a){},incomeparser:function(a){this.opts.income=a},success:function(a){return a}};!function(a){a.barchart=function(h,f){f=a.extend(true,{},f);var d=a(h);var e={data:{site:"http://openspending.org",dataset:f.data.dataset}};var k={drilldowns:f.data.drilldowns,cuts:f.data.cuts};var j=function(n){var l={values:_.map(n.children,function(p,o){return{label:p.label,values:Math.round(p.amount)}})};l.values.sort(function(p,o){return p.label-o.label});i.loadJSON(l)};d.height(200);d.width(200);var i=new $jit.BarChart({injectInto:h,animate:true,orientation:"vertical",barsOffset:5,Margin:{top:5,left:5,right:5,bottom:5},labelOffset:5,type:"stacked",showAggregates:true,showLabels:true,Label:{type:"Native",size:13,family:"Arial",color:"black"},Tips:{enable:true,onShow:function(n,l){n.innerHTML="<b>"+l.name+"</b>: "+l.value}}});var c=function(){k.drilldowns=k.drilldowns||[k.drilldown];k.cuts=k.cuts||{};var l=[];for(var n in k.cuts){l.push(n+":"+k.cuts[n])}k.drilldowns.unshift("year");var o=new OpenSpending.Aggregator();o.get({siteUrl:e.data.site,dataset:e.data.dataset,drilldowns:k.drilldowns,cuts:l,rootNodeLabel:"total",callback:j})};c()};a.fn.extend({barchart:function(c){if(c==undefined){c={}}this.each(function(){a.barchart(this,c)})}})}(jQuery);
